...
onKeyDown: function(evt) {
  ...
  if (t1.get$keyCode(evt) === 27) {
    t2 = $.game;
    t2.activeSymbol = -1;
    for (i = 0; t3 = t2.symbols, i < t3.length; ++i)
      t3[i].active = false;
    for (i = 0; t2 = $.game, t3 = t2.buildings, i < t3.length; ++i)
      J.set$selected$x(t3[i], false);
    for (i = 0; t3 = t2.ships, i < t3.length; ++i)
      t3[i].selected = false;
    t2 = $.engine.canvas;
...
onClickGUI: function(evt) {
  ...
  for (i = 0; t2 = t1.ships, i < t2.length; ++i)
    t2[i].selected = false;
...
onDoubleClick: function(evt) {
  var t1, t2, t3, i, selectShips;
  for (t1 = $.game, t2 = t1.ships, t3 = t2.length, i = 0; selectShips = false, i < t3; ++i)
    if (t2[i].hovered) {
      selectShips = true;
      break;
    }
...
doneResizing: function() {
  ...
  if (typeof width !== "number")
    throw width.$div();
  t1.halfWidth = C.JSNumber_methods.toInt$0(Math.floor(width / 2));
  t1 = $.engine;
  if (typeof height !== "number")
    throw height.$div();
  t1.halfHeight = C.JSNumber_methods.toInt$0(Math.floor(height / 2));
  t1 = $.engine.canvas;
          ...
          for (i = 0; t1 = $.game, t2 = t1.emitters, i < t2.length; ++i) {
            ...
            if (t1 + t2 <= Math.pow(t3 * t4, 2)) {
              ...
              if (t2.building == null) {
                ...
                if (i >= t1.length)
                  throw H.ioore(t1, i);
                this.weaponTargetPosition = t1[i].position;
                break;
              }
            }
          }
        ...
        for (i = 0; t1 = $.game.spores, i < t1.length; ++i) {
          ...
          if (t1 + t2 <= Math.pow(t3 * t4, 2)) {
            ...
            if (t2.health <= 0) {
              ...
              if (typeof t4 !== "number")
                throw t4.$tdiv();
              t4 = C.JSNumber_methods.$tdiv(t4, t1);
              t3 = t3.y;
              if (typeof t3 !== "number")
                throw t3.$tdiv();
              t2.playSound$2("explosion", new U.Vector(t4, C.JSNumber_methods.$tdiv(t3, t1)));
              t1 = $.game.explosions;
...
Emitter: {"": "Object;position>,imageID<,strength,building",
  ...
  spawn$0: function() {
    ...
    if (this.building == null) {
      ...
      if (typeof t2 !== "number")
        throw t2.$add();
      J.$index$asx(t3, t2 + 1).set$creep(this.strength);
    }
  },
  draw$0: function() {
...
Engine: {"": "Object;FPS,delta,fps_delta,fps_frames,fps_totalTime,fps_updateTime,fps_updateFrames,animationRequest,width>,height*,halfWidth,halfHeight,fps_lastTime,mouse,mouseGUI,canvas,sounds,images,resizeTimer",
  ...
  isVisible$2: function(position, size) {
    var object, t1, t2, t3, t4, t5, t6, t7, t8, view;
    ...
    if (typeof t5 !== "number")
      throw t5.$mul();
    view = new P.Rectangle(t3 * t4 - t6 * t1, t2 * t4 - t7 * t1, t8 * t1, t5 * t1);
...
Game: {"": "Object;tileSize,seed,currentEnergy,maxEnergy,activeSymbol,terraformingHeight,creeperCounter,collectCounter,speed,zoom,running,mode,paused,scrollingUp,scrollingDown,scrollingLeft,scrollingRight,ghosts,packetQueue,sporetowers,emitters,symbols,explosions,smokes,spores,buildings,packets,shells,ships,projectiles,world,scroll,base,stopwatch",
  ...
  createWorld$0: function() {
    var t1, i, t2, t3, j, heightmap, height, randomPosition, building, number, l, max, emitter, sporetower;
    t1 = this.world;
    ...
    for (l = 0; t1 = $.engine, l < number; ++l) {
      ...
      if (max > 4294967295)
        max = 4294967295;
      emitter = new U.Emitter(new U.Vector(t1 + 0, (Math.random() * max >>> 0) + 0), null, 25, null);
      ...
      if (typeof t2 !== "number")
        throw t2.$add();
      height = J.get$height$x(J.$index$asx(t3, t2 + 1));
      if (typeof height !== "number")
        throw height.$lt();
      if (height < 0)
        height = 0;
    ...
    number = t1.randomInt$3(1, 2, this.seed + 1);
    for (l = 0; l < number; ++l) {
      t1 = $.engine;
      t2 = this.world.size.x;
      if (typeof t2 !== "number")
  ...
  removeBuilding$1: function(building) {
    ...
    for (i = this.packets.length - 1; i >= 0; --i) {
      ...
      if (!J.$eq(t1[i].currentTarget, building)) {
        ...
        if (i >= t1.length)
          throw H.ioore(t1, i);
        t1 = t1[i].target;
        t1 = t1 == null ? building == null : t1 === building;
      } else
        t1 = true;
      if (t1)
        C.JSArray_methods.removeAt$1(this.packets, i);
    }
    for (i = this.packetQueue.length - 1; i >= 0; --i) {
      ...
      if (!J.$eq(t1[i].currentTarget, building)) {
        ...
        t1 = true;
      if (t1)
        C.JSArray_methods.removeAt$1(this.packetQueue, i);
    }
    t1 = this.buildings;
    index = H.Arrays_indexOf(t1, building, 0, t1.length);
    C.JSArray_methods.removeAt$1(this.buildings, index);
  },
  ...
  getNeighbours$2: function(node, target) {
    ...
    for (t1 = J.getInterceptor$x(node), t2 = this.tileSize, allowedDistance = 10 * t2, allowedDistance0 = 20 * t2, centerI = null, centerNode = null, i = 0; t2 = this.buildings, i < t2.length; ++i)
      if (!J.$eq(J.get$position$x(t2[i]), t1.get$position(node))) {
        t2 = this.buildings;
        if (i >= t2.length)
          throw H.ioore(t2, i);
        t2 = t2[i];
        if (J.get$status$x(t2) === "IDLE") {
          if (t2 == null ? target != null : t2 !== target) {
            t2 = this.buildings;
            if (i >= t2.length)
              throw H.ioore(t2, i);
            t2 = t2[i].get$built();
          ...
          if (t2) {
            ...
            if (typeof t4 !== "number")
              throw H.iae(t4);
            t3 = Math.pow(t3 - t4, 2);
            distance = Math.sqrt(t2 + t3);
            if (node.get$imageID() === "relay") {
              t2 = this.buildings;
              if (i >= t2.length)
                throw H.ioore(t2, i);
              t2 = t2[i].get$imageID() === "relay";
            } else
              t2 = false;
  ...
  inRoute$2: function(node, route) {
    var t1, i, t2;
    for (t1 = J.getInterceptor$x(node), i = 0; i < route.length; ++i) {
      t2 = t1.get$position(node);
      if (i >= route.length)
        throw H.ioore(route, i);
      if (J.$eq(t2, J.get$position$x(route[i])))
        return true;
    }
    return false;
  },
  findRoute$1: function(packet) {
    var routes, t1, route, t2, t3, oldRoute, neighbours, newRoutes, i, newRoute, t4, centerA, centerB, t5, centerC, j;
    ...
    for (; routes.length > 0;) {
      ...
      for (newRoutes = 0, i = 0; i < neighbours.length; ++i)
        if (!this.inRoute$2(neighbours[i], t1)) {
          ++newRoutes;
          ...
          if (typeof t5 !== "number")
            throw H.iae(t5);
          t4 = Math.pow(t4 - t5, 2);
          newRoute.distanceTravelled = t3 + Math.sqrt(t2 + t4);
  ...
  canBePlaced$3: function(position, size, building) {
    var t1, t2, t3, height, i, t4, collision, x1, t5, x2, y1, y2, cx1, cx2, cy1, cy2, j, tileHeight;
    t1 = position.x;
    if (typeof t1 !== "number")
      throw t1.$gt();
    if (t1 > -1) {
    ...
    if (t2) {
      ...
      for (t1 = this.tileSize, t2 = building != null, t3 = J.getInterceptor(building), i = 0; t4 = this.buildings, collision = false, i < t4.length; ++i) {
        t4 = t4[i];
        if (J.get$status$x(t4) !== "IDLE")
          continue;
        if (t2 && t3.$eq(building, t4))
          continue;
        t4 = this.buildings;
        if (i >= t4.length)
          throw H.ioore(t4, i);
        t4 = J.get$x$x(J.get$position$x(t4[i]));
        if (typeof t4 !== "number")
          throw t4.$mul();
        x1 = t4 * t1;
        t4 = this.buildings;
  ...
  updateCollection$2: function(collector, action) {
    var t1, t2, t3, height, centerBuilding, t4, t5, i, j, t6, t7, positionCurrent, positionCurrentCenter, t8, tileHeight, k, heightK, centerBuildingK, t9;
    t1 = $.game.world.tiles;
    ...
    for (t1 = action === "remove", t2 = action === "add", t3 = this.tileSize, t4 = t3 / 2, t5 = t3 * 6, i = -5; i < 7; ++i)
      for (j = -5; j < 7; ++j) {
        t6 = collector.position;
        t7 = t6.x;
        if (typeof t7 !== "number")
          throw t7.$add();
        t6 = t6.y;
        if (typeof t6 !== "number")
          throw t6.$add();
        positionCurrent = new U.Vector(t7 + i, t6 + j);
        if (this.withinWorld$2(positionCurrent.x, positionCurrent.y)) {
          t6 = positionCurrent.x;
          if (typeof t6 !== "number")
            throw t6.$mul();
          t7 = positionCurrent.y;
          if (typeof t7 !== "number")
            throw t7.$mul();
          positionCurrentCenter = new U.Vector(t6 * t3 + t4, t7 * t3 + t4);
          t8 = $.game.world.tiles;
          if (t6 >>> 0 !== t6 || t6 >= t8.length)
            throw H.ioore(t8, t6);
          tileHeight = J.get$height$x(J.$index$asx(t8[t6], t7));
          if (t2) {
            t6 = positionCurrentCenter.x;
            t7 = centerBuilding.x;
            if (typeof t6 !== "number")
              throw t6.$sub();
            if (typeof t7 !== "number")
            ...
            if (typeof t8 !== "number")
              throw H.iae(t8);
            t7 = Math.pow(t7 - t8, 2);
            if (t6 + t7 < Math.pow(t5, 2))
              if (J.$eq(tileHeight, height)) {
                t6 = this.world.tiles;
                t7 = positionCurrent.x;
                if (t7 >>> 0 !== t7 || t7 >= t6.length)
                  throw H.ioore(t6, t7);
                J.$index$asx(t6[t7], positionCurrent.y).set$collector(collector);
              }
          } else if (t1) {
            t6 = positionCurrentCenter.x;
            t7 = centerBuilding.x;
            ...
            if (t6 + t7 < Math.pow(t5, 2))
              if (J.$eq(tileHeight, height)) {
                ...
                if (t7 >>> 0 !== t7 || t7 >= t6.length)
                  throw H.ioore(t6, t7);
                J.$index$asx(t6[t7], positionCurrent.y).set$collector(null);
              }
            for (t6 = J.getInterceptor(tileHeight), k = 0; t7 = this.buildings, k < t7.length; ++k) {
              if (!J.$eq(t7[k], collector)) {
                t7 = this.buildings;
                if (k >= t7.length)
                  throw H.ioore(t7, k);
                t7 = t7[k].get$imageID() === "collector";
              } else
                t7 = false;
              if (t7) {
                t7 = $.game.world.tiles;
                ...
                if (k >= t8.length)
                  throw H.ioore(t8, k);
                t8 = J.get$x$x(J.get$position$x(t8[k]));
                ...
                if (k >= t7.length)
                  throw H.ioore(t7, k);
                heightK = J.get$height$x(J.$index$asx(t8, J.get$y$x(J.get$position$x(t7[k]))));
                t7 = this.buildings;
                if (k >= t7.length)
                  throw H.ioore(t7, k);
                centerBuildingK = t7[k].getCenter$0();
                t7 = positionCurrentCenter.x;
                t8 = centerBuildingK.x;
                if (typeof t7 !== "number")
                  throw t7.$sub();
                if (typeof t8 !== "number")
                  throw H.iae(t8);
                ...
                t9 = centerBuildingK.y;
                if (typeof t8 !== "number")
                  throw t8.$sub();
                if (typeof t9 !== "number")
                  throw H.iae(t9);
                t8 = Math.pow(t8 - t9, 2);
                if (t7 + t8 < Math.pow(t5, 2))
                  if (t6.$eq(tileHeight, heightK)) {
                    t7 = this.world.tiles;
                    t8 = positionCurrent.x;
                    if (t8 >>> 0 !== t8 || t8 >= t7.length)
                      throw H.ioore(t7, t8);
                    t8 = J.$index$asx(t7[t8], positionCurrent.y);
                    t7 = this.buildings;
                    if (k >= t7.length)
                      throw H.ioore(t7, k);
  ...
  updatePacketQueue$0: function() {
    ...
    for (i = this.packetQueue.length - 1; i >= 0; --i) {
      ...
      if (t1 > 0) {
        ...
        J.set$innerHtml$x(document.querySelector("#energy"), "Energy: " + C.JSInt_methods.toString$0(this.currentEnergy) + "/" + C.JSInt_methods.toString$0(this.maxEnergy));
        packet = C.JSArray_methods.removeAt$1(this.packetQueue, 0);
  ...
  drawPositionInfo$0: function() {
    var t1, context, t2, end, t3, t4, t5, delta, distance, buildingDistance, times, i, newX, newY, position, allowedDistance, allowedDistance0, j, positionScrolled, drawPosition, t6, t7, t8, t9, positionScrolledCenter, center, drawCenter, allowedDistance1, lineToTarget, k;
    t1 = $.engine.canvas;
    context = t1.$index(t1, "buffer").get$context();
    ...
    for (t1 = this.tileSize, t2 = t1 / 2, t3 = t2 * 3, allowedDistance = 10 * t1, allowedDistance0 = 20 * t1, t4 = J.getInterceptor$x(context), j = 0; t5 = this.ghosts, j < t5.length; ++j) {
      t5 = t5[j];
      positionScrolled = new U.Vector(t5.x, t5.y);
      drawPosition = positionScrolled.tiled2screen$0();
      t5 = positionScrolled.x;
      if (typeof t5 !== "number")
        throw t5.$mul();
      t6 = this.symbols;
      ...
      t6 = t7.size;
      t8 = t2 * t6;
      t9 = positionScrolled.y;
      if (typeof t9 !== "number")
        throw t9.$mul();
      positionScrolledCenter = new U.Vector(t5 * t1 + t8, t9 * t1 + t8);
      this.drawRangeBoxes$4(positionScrolled, t7.imageID, t7.radius, t6);
      if (this.withinWorld$2(positionScrolled.x, positionScrolled.y)) {
        ...
        if (typeof t8 !== "number")
          throw H.iae(t8);
        t8 = t9 * t1 * t8;
        context.drawImage(t7, t6, t5, t8, t8);
        t5 = this.symbols;
        t6 = this.activeSymbol;
        if (t6 >>> 0 !== t6 || t6 >= t5.length)
          throw H.ioore(t5, t6);
        if (t5[t6].imageID === "cannon") {
          t5 = $.engine.images;
          ...
          if (typeof t8 !== "number")
            throw H.iae(t8);
          t8 = 48 * t8;
          context.drawImage(t5, t6, t7, t8, t8);
        }
        t5 = this.symbols;
        t6 = this.activeSymbol;
        if (t6 >>> 0 !== t6 || t6 >= t5.length)
          throw H.ioore(t5, t6);
        if (this.canBePlaced$3(positionScrolled, t5[t6].size, null))
        ...
        if (typeof t7 !== "number")
          throw H.iae(t7);
        t7 = t1 * t8 * t7;
        context.strokeRect(t5, t6, t7, t7);
        context.restore();
        for (i = 0; t5 = this.buildings, i < t5.length; ++i) {
          ...
          if (t5[i].get$imageID() === "relay") {
            ...
            if (t6 >>> 0 !== t6 || t6 >= t5.length)
              throw H.ioore(t5, t6);
            t6 = t5[t6].imageID === "relay";
            t5 = t6;
          } else
            t5 = false;
          allowedDistance1 = t5 ? allowedDistance0 : allowedDistance;
          t5 = center.x;
          t6 = positionScrolledCenter.x;
          if (typeof t5 !== "number")
            throw t5.$sub();
          if (typeof t6 !== "number")
            throw H.iae(t6);
          t5 = Math.pow(t5 - t6, 2);
          t6 = center.y;
          t7 = positionScrolledCenter.y;
          if (typeof t6 !== "number")
            throw t6.$sub();
          if (typeof t7 !== "number")
            throw H.iae(t7);
          t6 = Math.pow(t6 - t7, 2);
          if (t5 + t6 <= Math.pow(allowedDistance1, 2)) {
        ...
        for (k = 0; t5 = this.ghosts, k < t5.length; ++k)
          if (k !== j) {
            t5 = t5[k];
            t6 = t5.x;
            if (typeof t6 !== "number")
            ...
            if (typeof t5 !== "number")
              throw t5.$mul();
            center = new U.Vector(t6 * t1 + t3, t5 * t1 + t3);
            drawCenter = center.real2screen$0();
            t5 = this.symbols;
            t6 = this.activeSymbol;
            if (t6 >>> 0 !== t6 || t6 >= t5.length)
              throw H.ioore(t5, t6);
            allowedDistance1 = t5[t6].imageID === "relay" ? allowedDistance0 : allowedDistance;
            t5 = center.x;
            t6 = positionScrolledCenter.x;
            if (typeof t5 !== "number")
              throw t5.$sub();
  ...
  draw$1: function(_) {
    ...
    for (i = 0; t1 = this.sporetowers, i < t1.length; ++i)
      t1[i].draw$0();
    for (allowedDistance = 10 * t2, allowedDistance0 = 20 * t2, i = 0; t1 = this.buildings, i < t1.length; ++i) {
      centerI = t1[i].getCenter$0();
      drawCenterI = centerI.real2screen$0();
      for (j = 0; t1 = this.buildings, t4 = t1.length, j < t4; ++j)
        if (i !== j) {
          if (i >= t4)
            throw H.ioore(t1, i);
          if (J.get$status$x(t1[i]) === "IDLE" && J.get$status$x(t1[j]) === "IDLE") {
            ...
            if (t1[i].get$imageID() === "relay") {
              if (j >= t4)
                throw H.ioore(t1, j);
              t1 = t1[j].get$imageID() === "relay";
            } else
              t1 = false;
            allowedDistance1 = t1 ? allowedDistance0 : allowedDistance;
            t1 = centerJ.x;
            t4 = centerI.x;
            if (typeof t1 !== "number")
              throw t1.$sub();
            ...
            if (typeof t5 !== "number")
              throw H.iae(t5);
            t4 = Math.pow(t4 - t5, 2);
            if (t1 + t4 <= Math.pow(allowedDistance1, 2)) {
    ...
    for (i = 0; t1 = this.shells, i < t1.length; ++i)
      t1[i].draw$0();
    for (i = 0; t1 = this.projectiles, i < t1.length; ++i)
      t1[i].draw$0();
    for (i = 0; t1 = this.buildings, i < t1.length; ++i)
      t1[i].draw$0();
    for (i = 0; t1 = this.smokes, i < t1.length; ++i)
      t1[i].draw$0();
    for (i = 0; t1 = this.explosions, i < t1.length; ++i)
      t1[i].draw$0();
    for (i = 0; t1 = this.spores, i < t1.length; ++i)
      t1[i].draw$0();
    if ($.engine.mouse.active) {
      ...
      if (this.mode === "SHIP_SELECTED") {
        ...
        if (typeof t7 !== "number")
          throw t7.$sub();
        t5 = 48 * t5;
        t3.drawImageScaled$5(context, t1, t4 - t6, t7 - t6, t5, t5);
      }
      ...
      if (this.mode === "TERRAFORM") {
        ...
        if (typeof t7 !== "number")
          throw H.iae(t7);
        t7 = t2 * t7;
        t3.drawImageScaledFromSource$9(context, t1, t4 * t2, 0, t2, t2, t5, t6, t7, t7);
        context.strokeStyle = "#fff";
        context.lineWidth = 1;
        context.beginPath();
