...
onMouseMove: function(evt) {
  ...
  t1 = $.game;
  if (t1 != null) {
    t1.scrollingLeft = J.$eq($.engine.mouse.position.x, 0);
    t1 = $.game;
    t2 = $.engine;
    t3 = t2.mouse.position.x;
    t2 = t2.width;
    if (typeof t2 !== "number")
      throw t2.$sub();
    t1.scrollingRight = J.$eq(t3, t2 - 1);
    $.game.scrollingUp = J.$eq($.engine.mouse.position.y, 0);
    t2 = $.game;
    t3 = $.engine;
  ...
  collectEnergy$0: function() {
    ...
    if (t1 >= 100) {
      ...
      if (this.type === "collector") {
        packet = U.Packet$(this.position, "packet_collection", "collection");
    ...
    for (t1 = action === "remove", t2 = action === "add", i = -5; i < 7; ++i)
      for (j = -5; j < 7; ++j) {
        ...
        if (t3.contains$1(t3, positionCurrent)) {
          ...
          if (t2) {
            t3 = J.$sub$n(positionCurrentCenter.x, centerBuilding.x);
            if (typeof t3 !== "number")
              H.throwExpression(new P.ArgumentError(t3));
            t3 = Math.pow(t3, 2);
            t4 = J.$sub$n(positionCurrentCenter.y, centerBuilding.y);
            if (typeof t4 !== "number")
              H.throwExpression(new P.ArgumentError(t4));
            t4 = Math.pow(t4, 2);
            t5 = $.game.tileSize;
            if (t3 + t4 < Math.pow(t5 * 6, 2))
              if (J.$eq(tileHeight, height)) {
                t3 = $.game.world.tiles;
                t4 = positionCurrent.x;
                if (t4 >>> 0 !== t4 || t4 >= t3.length)
            ...
            if (t3 + t4 < Math.pow(t5 * 6, 2))
              if (J.$eq(tileHeight, height)) {
                ...
                if (t4 >>> 0 !== t4 || t4 >= t3.length)
                  throw H.ioore(t3, t4);
                J.$index$asx(t3[t4], positionCurrent.y).set$collector(null);
              }
            for (t3 = J.getInterceptor(tileHeight), k = 0; k < $.get$Building_buildings().length; ++k) {
                ...
                if (t5 + t4 < Math.pow(t6 * 6, 2))
                  if (t3.$eq(tileHeight, heightK)) {
                    ...
                    if (t5 >>> 0 !== t5 || t5 >= t4.length)
                      throw H.ioore(t4, t5);
                    t5 = J.$index$asx(t4[t5], positionCurrent.y);
                    t4 = $.get$Building_buildings();
                    if (k >= t4.length)
                      throw H.ioore(t4, k);
                    t5.set$collector(t4[k]);
                  }
              }
            }
          }
...
Building_remove: function(building) {
  ...
  if (building.type === "storage") {
    ...
    t1.toString;
    J.set$innerHtml$x(document.querySelector("#energy"), "Energy: " + C.JSInt_methods.toString$0(t1.currentEnergy) + "/" + C.JSInt_methods.toString$0(t1.maxEnergy));
  }
...
Building_drawRepositionInfo: function() {
  ...
  for (t1 = J.getInterceptor$x(context), i = 0; i < $.get$Building_buildings().length; ++i) {
    ...
    if (t2) {
      ...
      if (i >= t4.length)
        throw H.ioore(t4, i);
      if (t2.canBePlaced$3(hoveredTilePosition, t3, t4[i]))
...
Game: {"": "Object;tileSize,seed,currentEnergy,maxEnergy<,terraformingHeight,creeperCounter,collectCounter,speed,zoom,running,mode,paused,scrollingUp,scrollingDown,scrollingLeft,scrollingRight,creeperDirty,ghosts,world,scroll,base,stopwatch,tfLine1,tfLine2,tfLine3,tfLine4,tfNumber,targetCursor",
  ...
  canBePlaced$3: function(position, size, building) {
    ...
    if (J.$gt$n(position.x, 0) && J.$lt$n(position.x, J.$sub$n(this.world.size.x, 1)) && J.$gt$n(position.y, 0) && J.$lt$n(position.y, J.$sub$n(this.world.size.y, 1))) {
      t1 = $.game.world.tiles;
      t2 = position.x;
      if (t2 >>> 0 !== t2 || t2 >= t1.length)
        throw H.ioore(t1, t2);
      height = J.get$height$x(J.$index$asx(t1[t2], position.y));
      for (t1 = this.tileSize, t2 = J.getInterceptor$n(size), t3 = building != null, t4 = J.getInterceptor(building), i = 0; collision = false, i < $.get$Building_buildings().length; ++i) {
        t5 = $.get$Building_buildings();
        if (i >= t5.length)
          throw H.ioore(t5, i);
        if (J.get$status$x(t5[i]) !== "IDLE")
          continue;
        if (t3) {
          t5 = $.get$Building_buildings();
          if (i >= t5.length)
            throw H.ioore(t5, i);
          t5 = t4.$eq(building, t5[i]);
        } else
          t5 = false;
        if (t5)
          continue;
        ...
        if (i >= t5.length)
          throw H.ioore(t5, i);
        t5 = J.get$x$x(J.get$position$x(t5[i]));
        t6 = $.get$Building_buildings();
        if (i >= t6.length)
          throw H.ioore(t6, i);
        t6 = J.$sub$n(t5, J.$div$n(J.$mul$n(J.get$size$x(t6[i]), t1), 2));
        t5 = $.get$Building_buildings();
        if (i >= t5.length)
          throw H.ioore(t5, i);
        t5 = J.get$y$x(J.get$position$x(t5[i]));
        t7 = $.get$Building_buildings();
        if (i >= t7.length)
          throw H.ioore(t7, i);
        t7 = J.$sub$n(t5, J.$div$n(J.$mul$n(J.get$size$x(t7[i]), t1), 2));
        t5 = $.get$Building_buildings();
        if (i >= t5.length)
          throw H.ioore(t5, i);
        t5 = J.$sub$n(J.$mul$n(J.get$size$x(t5[i]), t1), 1);
        t8 = $.get$Building_buildings();
        if (i >= t8.length)
          throw H.ioore(t8, i);
      ...
      if (!collision)
        for (i = J.$sub$n(position.x, 1), collision = false; t1 = J.getInterceptor$n(i), t1.$le(i, J.$add$ns(position.x, 1)); i = t1.$add(i, 1))
          for (j = J.$sub$n(position.y, 1); t2 = J.getInterceptor$n(j), t2.$le(j, J.$add$ns(position.y, 1)); j = t2.$add(j, 1)) {
            t3 = this.world;
            if (t3.contains$1(t3, new U.Vector(i, j))) {
              t3 = $.game.world.tiles;
              if (i >>> 0 !== i || i >= t3.length)
              ...
              t3 = J.getInterceptor$n(tileHeight);
              if (t3.$lt(tileHeight, 0)) {
                collision = true;
                break;
              }
              ...
              if (i >= t3.length)
                throw H.ioore(t3, i);
              if (J.get$index$x(J.$index$asx(t3[i], j)) !== 7) {
                t3 = this.world.tiles;
                if (i >= t3.length)
                  throw H.ioore(t3, i);
                if (J.get$index$x(J.$index$asx(t3[i], j)) !== 11) {
                  t3 = this.world.tiles;
                  if (i >= t3.length)
                    throw H.ioore(t3, i);
                  if (J.get$index$x(J.$index$asx(t3[i], j)) !== 13) {
                    t3 = this.world.tiles;
                    if (i >= t3.length)
                      throw H.ioore(t3, i);
                    if (J.get$index$x(J.$index$asx(t3[i], j)) !== 14) {
                      t3 = this.world.tiles;
                      if (i >= t3.length)
                        throw H.ioore(t3, i);
                      t3 = J.get$index$x(J.$index$asx(t3[i], j)) === 15;
                    } else
                      t3 = true;
                  } else
                    t3 = true;
  ...
  update$0: function() {
    ...
    if (this.scrollingLeft) {
      if (J.$gt$n(this.scroll.x, 0)) {
        var t1 = this.scroll;
        t1.x = J.$sub$n(t1.x, 1);
      }
    } else if (this.scrollingRight)
      if (J.$lt$n(this.scroll.x, this.world.size.x)) {
        t1 = this.scroll;
        t1.x = J.$add$ns(t1.x, 1);
      }
    if (this.scrollingUp) {
      if (J.$gt$n(this.scroll.y, 0)) {
        t1 = this.scroll;
        t1.y = J.$sub$n(t1.y, 1);
      }
    } else if (this.scrollingDown)
      if (J.$lt$n(this.scroll.y, this.world.size.y)) {
        t1 = this.scroll;
      ...
      if (J.$eq(this.currentTarget, this.target)) {
        ...
        if (t1 === "health") {
          ...
          if (t2 >= t3) {
            ...
            if (!t1.built) {
              t1.built = true;
              this.target.sprite.alpha = 1;
              U.Connection_activate(this.target);
              t1 = this.target.cannon;
              if (t1 != null)
                t1.alpha = 1;
              t1 = this.target;
              if (t1.type === "collector") {
                t1.updateCollection$1("add");
                $.engine.playSound$2("energy", this.target.position.real2tiled$0());
              }
    ...
    for (; routes.length > 0;) {
      ...
      for (newRoutes = 0, i = 0; i < neighbours.length; ++i)
        if (!oldRoute.contains$1(oldRoute, neighbours[i])) {
          ...
          if (t3 < 0)
            throw H.ioore(t1, t3);
          centerA = J.get$position$x(t1[t3]);
          t3 = t1.length;
          t2 = t3 - 2;
          if (t2 < 0)
            throw H.ioore(t1, t2);
      ...
      t1 = new U.Packet_findRoute_closure();
      t2 = routes.length - 1;
      if (t2 - 0 <= 32)
...
Packet$: function(position, imageID, type) {
  var t1 = new U.Packet(new U.Vector(0, 0), type, false, 1, null, null, null);
  t1.Packet$3(position, imageID, type);
  return t1;
},
...
Packet_queuePacket: function(target, type) {
  var img, packet, t1;
  img = "packet_" + type;
  packet = U.Packet$($.game.base.position, img, type);
...
function dart_precompiled($collectedClasses) {
  ...
  Building.prototype.get$sprite = function() {
    return this.sprite;
  };
  ...
  Connection.prototype.get$line = function() {
    return this.line;
  };
  Connection.prototype.get$line2 = function() {
    return this.line2;
  };
  function DisplayObject(layer, visible) {
    this.layer = layer;
  ...
  DisplayObject.prototype.get$layer = function(receiver) {
    return this.layer;
  };
  ...
  Emitter.prototype.get$sprite = function() {
    return this.sprite;
  };
  function Mouse(position, active, dragStart, dragEnd) {
    this.position = position;
    this.active = active;
  ...
  function Engine(animationRequest, TPS, width, height, halfWidth, halfHeight, mouse, mouseGUI, canvas, sounds, images, resizeTimer) {
  ...
  Explosion.prototype.get$sprite = function() {
    return this.sprite;
  };
  function Game(tileSize, seed, currentEnergy, maxEnergy, terraformingHeight, creeperCounter, collectCounter, speed, zoom, running, mode, paused, scrollingUp, scrollingDown, scrollingLeft, scrollingRight, creeperDirty, ghosts, world, scroll, base, stopwatch, tfLine1, tfLine2, tfLine3, tfLine4, tfNumber, targetCursor) {
  ...
  if ($desc instanceof Array)
    $desc = $desc[1];
  Game.prototype = $desc;
  Game.prototype.get$maxEnergy = function() {
    return this.maxEnergy;
  };
  ...
  Packet.prototype.get$sprite = function() {
    return this.sprite;
  };
  ...
  Projectile.prototype.get$sprite = function() {
    return this.sprite;
  };
  function Renderer(view, context, top, left, bottom, right, displayObjects) {
    this.view = view;
  ...
  function Renderer_addDisplayObject_closure() {
  }
  Renderer_addDisplayObject_closure.builtin$cls = "Renderer_addDisplayObject_closure";
  if (!"name" in Renderer_addDisplayObject_closure)
    Renderer_addDisplayObject_closure.name = "Renderer_addDisplayObject_closure";
  $desc = $collectedClasses.Renderer_addDisplayObject_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Renderer_addDisplayObject_closure.prototype = $desc;
  ...
  Shell.prototype.get$sprite = function() {
    return this.sprite;
  };
  function Ship(speed, targetPosition, type, status, remove, hovered, selected, maxEnergy, energy, trailCounter, weaponCounter, flightCounter, home, sprite, targetSymbol, hoverCircle, selectedCircle) {
    this.speed = speed;
    this.targetPosition = targetPosition;
    this.type = type;
    this.status = status;
  ...
  Ship.prototype.get$sprite = function() {
    return this.sprite;
  };
  ...
  Smoke.prototype.get$sprite = function() {
    return this.sprite;
  };
  ...
  Spore.prototype.get$sprite = function() {
    return this.sprite;
  };
  ...
  Sporetower.prototype.get$sprite = function() {
    return this.sprite;
  };
  ...
  Vector3.prototype.set$x = function(receiver, v) {
    return this.x = v;
  };
  Vector3.prototype.get$y = function(receiver) {
    return this.y;
  };
  Vector3.prototype.set$y = function(receiver, v) {
    return this.y = v;
  };
