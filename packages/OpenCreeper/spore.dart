...
class Spore {
  ...
  static void update() {
    for (int i = spores.length - 1; i >= 0; i--) {
      if (spores[i].remove) {
        game.engine.renderer["buffer"].removeDisplayObject(spores[i].sprite);
        spores.removeAt(i);
      }
      else
        spores[i].move();
    ...
    for (int i = 0; i < spores.length; i++) {
      ...
      if (distance <= pow(building.weaponRadius * game.tileSize, 2)) {
        ...
        if (spores[i].health <= 0) {
          ...
          game.engine.playSound("explosion", spores[i].sprite.position.real2tiled());
  ...
  void move() {
    ...
    if (sprite.position == targetPosition) {
      ...
      game.engine.playSound("explosion", targetPosition.real2tiled());
