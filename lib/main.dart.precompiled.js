...
["creeper", "main.dart", , U, {
  ...
  Building: {
    ...
    move$0: function() {
      ...
      if (this.status === "MOVING") {
        ...
        this.position = t1;
        if (J.$gt$n(t1.x, J.$sub$n(this.moveTargetPosition.x, 1)) && J.$lt$n(this.position.x, J.$add$ns(this.moveTargetPosition.x, 1)) && J.$gt$n(this.position.y, J.$sub$n(this.moveTargetPosition.y, 1)) && J.$lt$n(this.position.y, J.$add$ns(this.moveTargetPosition.y, 1))) {
      ...
      if (this.status === "IDLE") {
        for (i = -C.JSInt_methods.$tdiv(this.size, 2); t1 = C.JSInt_methods.$tdiv(this.size, 2), i <= t1; ++i)
          for (j = -t1; j <= -C.JSInt_methods.$tdiv(this.size, 2); ++j) {
            t1 = this.position;
            t2 = J.$tdiv$n(t1.x, $.game.tileSize);
            t1 = J.$tdiv$n(t1.y, $.game.tileSize);
            t2 = J.$add$ns(t2, i);
            t1 = J.$add$ns(t1, j);
            t3 = $.game.world;
            if (t3.contains$1(t3, new U.Vector(t2, t1))) {
              t1 = $.game;
              t2 = t1.world;
              t3 = this.position;
              t1 = t1.tileSize;
              ...
              if (t4 >>> 0 !== t4 || t4 >= t2.length)
                throw H.ioore(t2, t4);
              tile = J.$index$asx(t2[t4], J.$tdiv$n(t1, 16));
              if (tile.get$creep() > 0)
    ...
    collectEnergy$0: function() {
      var height, centerBuilding, i, j, t1, t2, positionCurrent, t3, t4, tileHeight, packet;
      if (this.type === "collector" && this.built) {
        ...
        for (i = -5; i < 7; ++i)
          for (j = -5; j < 7; ++j) {
            ...
            t2 = J.$tdiv$n(t1.x, $.game.tileSize);
            t1 = J.$tdiv$n(t1.y, $.game.tileSize);
            positionCurrent = new U.Vector(J.$add$ns(t2, i), J.$add$ns(t1, j));
            t1 = $.game.world;
    ...
    updateCollection$1: function(action) {
      var height, t1, t2, i, j, t3, t4, positionCurrent, positionCurrentCenter, t5, t6, tileHeight, k, heightK;
      height = J.get$height$x($.game.world.getTile$1(this.position));
      for (t1 = action === "remove", t2 = action === "add", i = -5; i < 7; ++i)
        for (j = -5; j < 7; ++j) {
          t3 = this.position;
          t4 = J.$tdiv$n(t3.x, $.game.tileSize);
          t3 = J.$tdiv$n(t3.y, $.game.tileSize);
          positionCurrent = new U.Vector(J.$add$ns(t4, i), J.$add$ns(t3, j));
          t3 = $.game.world;
    ...
    checkOperating$0: function() {
      var t1, positionTiled, i, lowestTile, j, tilePosition, t2, tileHeight, t3, t4, terraformElement, height, tilesToRedraw, t5, targets, targetPositionTiled, closestDistance, distance, dx, dy, absoluteDelta, turnRate, projectile, tiledPosition, target, highestCreep, t6, shell;
      ...
      if (this.built && this.needsEnergy && this.active && this.status === "IDLE") {
        ...
        if (t1 === "terp" && this.energy > 0) {
          ...
          if (t1 == null) {
            ...
            if (t4 >>> 0 !== t4 || t4 >= t2.length)
              throw H.ioore(t2, t4);
            terraformElement = J.$index$asx(t2[t4], t3.get$y(t1));
            ...
            if (t1 === 100) {
              ...
              if (t2) {
                ...
                if (t5 >>> 0 !== t5 || t5 >= t2.length)
                  throw H.ioore(t2, t5);
                t4 = J.$index$asx(t2[t5], t3.get$y(t4));
                t3 = J.getInterceptor$x(t4);
                t3.set$height(t4, J.$add$ns(t3.get$height(t4), 1));
                t4 = this.weaponTargetPosition;
                t3 = J.getInterceptor$x(t4);
                tilesToRedraw.push(new U.Vector3(t3.get$x(t4), t3.get$y(t4), t1.$add(height, 1)));
                ...
                t4 = this.weaponTargetPosition;
                t3 = J.getInterceptor$x(t4);
                tilesToRedraw.push(new U.Vector3(t3.get$x(t4), J.$sub$n(t3.get$y(t4), 1), t1.$add(height, 1)));
                ...
                t4 = this.weaponTargetPosition;
                t3 = J.getInterceptor$x(t4);
                tilesToRedraw.push(new U.Vector3(t3.get$x(t4), J.$add$ns(t3.get$y(t4), 1), t1.$add(height, 1)));
              } else {
                t2 = t3.world.tiles;
                t3 = J.getInterceptor$x(t4);
                t5 = t3.get$x(t4);
                ...
                t4 = J.$index$asx(t2[t5], t3.get$y(t4));
                t3 = J.getInterceptor$x(t4);
                t3.set$height(t4, J.$sub$n(t3.get$height(t4), 1));
                t4 = this.weaponTargetPosition;
                t3 = J.getInterceptor$x(t4);
                tilesToRedraw.push(new U.Vector3(t3.get$x(t4), t3.get$y(t4), height));
                ...
                t4 = this.weaponTargetPosition;
                t3 = J.getInterceptor$x(t4);
                tilesToRedraw.push(new U.Vector3(t3.get$x(t4), J.$sub$n(t3.get$y(t4), 1), height));
                ...
                t4 = this.weaponTargetPosition;
                t3 = J.getInterceptor$x(t4);
                tilesToRedraw.push(new U.Vector3(t3.get$x(t4), J.$add$ns(t3.get$y(t4), 1), height));
              }
    ...
    static: {"": "Building_baseSpeed,Building_damageCounter,Building_collectCounter,Building_buildings,Building_base", Building$template: function(imageID) {
        ...
        t1 = $.get$Building_buildings();
        J.removeAt$1$ax(t1, H.Arrays_indexOf(t1, building, 0, t1.length));
        ...
        for (i = 0; t1 = $.get$Building_buildings(), i < t1.length; ++i) {
          ...
          if (typeof t2 !== "number")
            throw H.iae(t2);
          if (J.$gt$n(t4, J.$sub$n(t5, t6 * t1 * t2 / 2))) {
            ...
            if (typeof t6 !== "number")
              throw H.iae(t6);
            if (J.$lt$n(t1, J.$add$ns(t2, t4 * t5 * t6 / 2))) {
              ...
              if (typeof t6 !== "number")
                throw H.iae(t6);
              if (J.$gt$n(t1, J.$sub$n(t2, t4 * t5 * t6 / 2))) {
                t1 = $.engine.mouse.position.y;
                t2 = realPosition.y;
                t4 = $.game.tileSize;
                t5 = $.get$Building_buildings();
                ...
                if (typeof t6 !== "number")
                  throw H.iae(t6);
                t6 = J.$lt$n(t1, J.$add$ns(t2, t4 * t5 * t6 / 2));
                t1 = t6;
              } else
                t1 = false;
            } else
        ...
        var t1, context, i, t2, hoveredTilePosition, t3, t4, positionI, t5, canBePlaced, t6, t7, t8, t9, j, positionJ, allowedDistance;
        t1 = $.engine.renderer;
        ...
        for (t1 = J.getInterceptor$x(context), i = 0; t2 = $.get$Building_buildings(), i < t2.length; ++i) {
          ...
          if (t2.get$built() && t2.selected && t2.canMove) {
            ...
            if (i >= t5.length)
              throw H.ioore(t5, i);
            t2.drawRangeBoxes$2(hoveredTilePosition, t5[i]);
            t5 = $.game;
            t2 = $.get$Building_buildings();
            if (i >= t2.length)
              throw H.ioore(t2, i);
            canBePlaced = t5.canBePlaced$2(hoveredTilePosition, t2[i]);
            if (canBePlaced)
        ...
        if (t2 >= 25) {
          ...
          for (i = 0; t1 = $.get$Emitter_emitters(), i < t1.length; ++i) {
            ...
            if (t1.analyzer == null) {
              ...
              t2 = t2.tiles;
              t3 = J.$tdiv$n(J.get$x$x(t1), 16);
              if (t3 >>> 0 !== t3 || t3 >= t2.length)
                throw H.ioore(t2, t3);
              t1 = J.$index$asx(t2[t3], J.$tdiv$n(t1.y, 16));
              t3 = t1.get$creep();
  ...
  Mouse: {
    "": "Object;position>,active<,buttonPressed,dragStart,dragEnd",
    toString$0: function(_) {
  ...
  Engine: {
    ...
    setupEventHandler$0: function() {
      ...
      H.setRuntimeTypeInfo(t4, [H.getTypeArgumentByIndex(t1, 0)]);
      t4._tryResume$0();
      ...
      H.setRuntimeTypeInfo(t1, [H.getTypeArgumentByIndex(t4, 0)]);
      t1._tryResume$0();
  ...
  Engine_setupEventHandler_closure5: {
    "": "Closure;",
    call$1: function($event) {
      var t1, t2, t3, t4, t5, hoveredTilePosition, currentRect;
      t1 = $.engine;
      t2 = t1.mouse;
      t3 = t2.position;
      ...
      t5 = t5.get$x(t5);
      t1 = t1.renderer;
      t3.x = J.toInt$0$n(J.$sub$n(t5, J.get$left$x(J.getBoundingClientRect$0$x(J.get$view$x(t1.$index(t1, "main"))))));
      t5 = t2.position;
      t4 = t4.get$client($event);
      t5.y = J.toInt$0$n(J.$sub$n(t4.get$y(t4), J.get$top$x(J.getBoundingClientRect$0$x(J.get$view$x(t1.$index(t1, "main"))))));
      t1 = $.game;
      if (t1 != null) {
        t2.dragEnd = t1.getHoveredTilePosition$0();
  ...
  Engine_setupEventHandler_closure7: {
    ...
    call$1: function($event) {
      ...
      $.engine.mouse.buttonPressed = J.get$which$x($event);
      if ($event.which === 1) {
        hoveredTilePosition = $.game.getHoveredTilePosition$0();
        t1 = $.engine.mouse;
  ...
  Engine_setupEventHandler_closure12: {
    "": "Closure;",
    call$1: function($event) {
      var t1, t2, t3, position, tilesToRedraw;
      ...
      if (t1.get$keyCode($event) === 113) {
        ...
        t3 = t2.speed;
        if (t3 > 1) {
          t2.speed = C.JSNumber_methods.$tdiv(t3, 2);
          t2.updateSpeedElement$0();
        }
        t1.preventDefault$0($event);
      }
  ...
  Engine_loadImages_closure: {
    "": "Closure;box_0,this_1,completer_2,filenames_3",
  ...
  Engine_loadImages__closure: {
    "": "Closure;box_0,completer_4,filenames_5",
    call$1: function($event) {
      var t1, loadedImages;
      t1 = this.box_0;
  ...
  Game: {
    "": "Object;tileSize,seed,terraformingHeight,speed,zoom,running,mode,paused,won,ghosts,world,scroll,mouseScrolling,keyScrolling,stopwatch,tfLine1,tfLine2,tfLine3,tfLine4,tfNumber,targetCursor",
    init$0: function() {
      var t1, music, t2;
      ...
      t2 = $.engine.images;
      t2 = U.Sprite$(C.Layer_0, t2.$index(t2, "numbers"), new U.Vector(0, 0), 16, 16);
      this.tfNumber = t2;
      t2.visible = false;
      t2.animated = true;
      t2.frame = this.terraformingHeight;
      t2 = $.engine.renderer;
      t2.$index(t2, "buffer").addDisplayObject$1(this.tfNumber);
      t2 = $.engine.images;
      t2 = U.Sprite$(C.Layer_0, t2.$index(t2, "targetcursor"), new U.Vector(0, 0), 48, 48);
      this.targetCursor = t2;
      t2.anchor = new U.Vector(0.5, 0.5);
      t2.visible = false;
      t2 = $.engine.renderer;
    ...
    createWorld$0: function() {
      ...
      t1 = this.seed;
      heightmap = new U.HeightMap(t1, 129, 0, 90, null, null, P.List_List(null, null), null);
      ...
      for (t1 = this.tileSize, i = -4; i <= 4; ++i)
        for (t2 = i * t1, j = -4; j <= 4; ++j) {
          ...
          if (t5 >>> 0 !== t5 || t5 >= t3.length)
            throw H.ioore(t3, t5);
          J.set$height$x(J.$index$asx(t3[t5], J.$tdiv$n(t4, 16)), height);
        }
      ...
      for (l = 0; t2 = $.engine, l < number; ++l) {
        ...
        emitter.Emitter$2(new U.Vector((t2 + 1) * t1 + 8, ((Math.random() * max >>> 0) + 1) * t1 + 8), 25);
      ...
      for (l = 0; l < number; ++l) {
        ...
        sporetower.Sporetower$1(new U.Vector((t2 + 1) * t1 + 8, ((Math.random() * max >>> 0) + 1) * t1 + 8));
    ...
    redrawTerrain$1: function(tiles) {
      var tempCanvas, tempContext, t1, t, e, t2, t3, t4, i, t5, iS, jS, index, indexAbove, t6, up, down, left, t7, right, t8, t9, pattern;
      tempCanvas = [];
      tempContext = [];
    ...
    canBePlaced$2: function(position, building) {
      var t1, t2, height, currentRect, t3, i, t4, j, t5, tileHeight;
      t1 = $.game.world;
      if (t1.contains$1(t1, position)) {
        ...
        if (typeof t3 !== "number")
          throw t3.$tdiv();
        i = J.$sub$n(t2, C.JSInt_methods.$tdiv(t3, 2));
        while (true) {
          t2 = position.x;
          t3 = t1.get$size(building);
          if (typeof t3 !== "number")
            throw t3.$tdiv();
          t4 = J.getInterceptor$n(i);
          if (!t4.$le(i, J.$add$ns(t2, C.JSInt_methods.$tdiv(t3, 2))))
            break;
          t2 = position.y;
          t3 = t1.get$size(building);
          if (typeof t3 !== "number")
            throw t3.$tdiv();
          j = J.$sub$n(t2, C.JSInt_methods.$tdiv(t3, 2));
          while (true) {
            t2 = position.y;
            t3 = t1.get$size(building);
            if (typeof t3 !== "number")
              throw t3.$tdiv();
            t5 = J.getInterceptor$n(j);
            if (!t5.$le(j, J.$add$ns(t2, C.JSInt_methods.$tdiv(t3, 2))))
              break;
            t2 = this.world;
            if (t2.contains$1(t2, new U.Vector(i, j))) {
              ...
              t2 = J.getInterceptor$n(tileHeight);
              if (t2.$lt(tileHeight, 0) || !t2.$eq(tileHeight, height))
                return false;
              t2 = this.world.tiles;
              if (i >= t2.length)
                throw H.ioore(t2, i);
            ...
            if (t3.contains$1(t3, positionCurrent)) {
              ...
              if (Math.sqrt(t3 + t4) < building.weaponRadius * t1) {
                ...
                if (!(building.type === "collector" && !J.$eq(positionCurrentHeight, positionHeight)))
                  t3 = building.type === "cannon" && J.$gt$n(positionCurrentHeight, positionHeight);
                else
                  t3 = true;
                if (t3)
        ...
        for (t4 = i * t2, j0 = j; j0 <= timesY; ++j0) {
          ...
          if (t5.contains$1(t5, position)) {
            ...
            if (J.$index$asx(t5[t6], position.y).get$collector() != null) {
              if (J.$lt$n(J.$sub$n(position.y, 1), 0))
                up = 0;
              else {
                t5 = this.world.tiles;
                t6 = position.x;
              ...
              if (J.$gt$n(J.$add$ns(position.y, 1), J.$sub$n(this.world.size.y, 1)))
                down = 0;
              else {
                t5 = this.world.tiles;
                t6 = position.x;
              ...
              if (J.$lt$n(J.$sub$n(position.x, 1), 0))
                left = 0;
              else {
                t5 = this.world.tiles;
              ...
              if (J.$gt$n(J.$add$ns(position.x, 1), J.$sub$n(this.world.size.x, 1)))
                right = 0;
              else {
                t5 = this.world.tiles;
              ...
              if (J.$index$asx(t4[t5], position.y).get$creep() > t) {
                if (J.$lt$n(J.$sub$n(position.y, 1), 0))
                  ...
                  if (!(J.$index$asx(t4[t5], J.$sub$n(position.y, 1)).get$creep() > t)) {
                    ...
                    if (t5 >>> 0 !== t5 || t5 >= t4.length)
                      throw H.ioore(t4, t5);
                    t5 = J.$gt$n(J.get$height$x(J.$index$asx(t4[t5], J.$sub$n(position.y, 1))), height);
                    t4 = t5;
                  } else
                    t4 = true;
                  up = t4 ? 1 : 0;
                ...
                if (J.$gt$n(J.$add$ns(position.y, 1), J.$sub$n(this.world.size.y, 1)))
                  ...
                  if (!(J.$index$asx(t4[t5], J.$add$ns(position.y, 1)).get$creep() > t)) {
                    ...
                    if (t5 >>> 0 !== t5 || t5 >= t4.length)
                      throw H.ioore(t4, t5);
                    t5 = J.$gt$n(J.get$height$x(J.$index$asx(t4[t5], J.$add$ns(position.y, 1))), height);
                    t4 = t5;
                  } else
                    t4 = true;
                  down = t4 ? 1 : 0;
                ...
                if (J.$lt$n(J.$sub$n(position.x, 1), 0))
                  ...
                  if (!(J.$index$asx(t4[t5], position.y).get$creep() > t)) {
                    ...
                    if (t5 >>> 0 !== t5 || t5 >= t4.length)
                      throw H.ioore(t4, t5);
                    t5 = J.$gt$n(J.get$height$x(J.$index$asx(t4[t5], position.y)), height);
                    t4 = t5;
                  } else
                    t4 = true;
                  left = t4 ? 1 : 0;
                ...
                if (J.$lt$n(J.$sub$n(position.y, 1), 0))
                  ...
                  if (J.$index$asx(t4[t5], J.$sub$n(position.y, 1)).get$creep() > t)
                    if (indexOk) {
                      ...
                      if (t5 >>> 0 !== t5 || t5 >= t4.length)
                        throw H.ioore(t4, t5);
                      t5 = J.$lt$n(J.get$height$x(J.$index$asx(t4[t5], J.$sub$n(position.y, 1))), height);
                      t4 = t5;
                    } else
                      t4 = false;
                  else
                ...
                if (J.$gt$n(J.$add$ns(position.y, 1), J.$sub$n(this.world.size.y, 1)))
                  ...
                  if (J.$index$asx(t4[t5], J.$add$ns(position.y, 1)).get$creep() > t)
                    if (indexOk) {
                      ...
                      if (t5 >>> 0 !== t5 || t5 >= t4.length)
                        throw H.ioore(t4, t5);
                      t5 = J.$lt$n(J.get$height$x(J.$index$asx(t4[t5], J.$add$ns(position.y, 1))), height);
                      t4 = t5;
                    } else
                      t4 = false;
                  else
                ...
                if (J.$lt$n(J.$sub$n(position.x, 1), 0))
                  ...
                  if (J.$index$asx(t4[t5], position.y).get$creep() > t)
                    if (indexOk) {
                      ...
                      if (t5 >>> 0 !== t5 || t5 >= t4.length)
                        throw H.ioore(t4, t5);
                      t5 = J.$lt$n(J.get$height$x(J.$index$asx(t4[t5], position.y)), height);
                      t4 = t5;
                    } else
                      t4 = false;
                  else
    ...
    updateTerraformInfo$0: function() {
      var position, t1, t2, t3, t4;
      position = this.getHoveredTilePosition$0();
      t1 = this.world;
      if (t1.contains$1(t1, position)) {
        if (this.mode === "TERRAFORM") {
          t1 = this.tileSize;
          t2 = J.$mul$n(position.x, t1);
          t3 = J.$mul$n(position.y, t1);
          t4 = this.tfLine1;
          t4.from = new U.Vector(0, t3);
          t4.to = new U.Vector(J.$mul$n(this.world.size.y, t1), t3);
          t4.visible = true;
          t4 = this.tfLine2;
          t4.from = new U.Vector(0, J.$add$ns(t3, t1));
          t4.to = new U.Vector(J.$mul$n(this.world.size.y, t1), J.$add$ns(t3, t1));
          t4.visible = true;
          t4 = this.tfLine3;
          t4.from = new U.Vector(t2, 0);
          t4.to = new U.Vector(t2, J.$mul$n(this.world.size.y, t1));
          t4.visible = true;
          t4 = this.tfLine4;
          t4.from = new U.Vector(J.$add$ns(t2, t1), 0);
          t4.to = new U.Vector(J.$add$ns(t2, t1), J.$mul$n(this.world.size.y, t1));
          t4.visible = true;
          t4 = this.tfNumber;
          t4.position = new U.Vector(J.$mul$n(position.x, t1), J.$mul$n(position.y, t1));
          t4.visible = true;
        } else {
    ...
    drawPositionInfo$0: function() {
      var t1, context, t2, end, delta, distance, buildingDistance, times, i, newX, newY, position, allowedDistance, allowedDistance0, t3, positionScrolled, drawPosition, ghostICenter, t4, t5, t6, t7, ghostCanBePlaced, j, buildingCenter, allowedDistance1, t8;
  ...
  Game_run_closure: {
    ...
    call$1: function(timer) {
      ...
      if (J.$lt$n(t1.scroll.x, 0))
        t1.scroll.x = 0;
      else if (J.$gt$n(t1.scroll.x, t1.world.size.x))
        t1.scroll.x = t1.world.size.x;
      t2 = t1.scroll;
      t2.y = J.$add$ns(t2.y, J.$add$ns(t1.mouseScrolling.y, t5.y));
      if (J.$lt$n(t1.scroll.y, 0))
        t1.scroll.y = 0;
      else if (J.$gt$n(t1.scroll.y, t1.world.size.y))
        t1.scroll.y = t1.world.size.y;
  ...
  HeightMap_diamond_square_closure: {
    "": "Closure;box_0,this_1,left_2,top_3,random_4",
    call$0: function() {
      var t1 = this.box_0;
  ...
  HeightMap_diamond_square_closure0: {
    "": "Closure;box_0,this_5,top_6,right_7,random_8",
    call$0: function() {
      var t1 = this.box_0;
  ...
  HeightMap_diamond_square_closure1: {
    "": "Closure;box_0,this_9,left_10,bottom_11,random_12",
    call$0: function() {
      var t1 = this.box_0;
  ...
  HeightMap_diamond_square_closure2: {
    "": "Closure;box_0,this_13,right_14,bottom_15,random_16",
    call$0: function() {
      var t1 = this.box_0;
  ...
  main_closure: {
    ...
    call$1: function(results) {
      var t1 = new U.Game(16, null, 0, 1, 1, null, null, false, false, P.List_List(null, U.Vector), null, new U.Vector(0, 0), new U.Vector(0, 0), new U.Vector(0, 0), new P.Stopwatch(null, null), null, null, null, null, null, null);
          ...
          for (j = -1; j <= 1; ++j) {
            ...
            if (t1.contains$1(t1, tilePosition)) {
              ...
              if (t1 <= t3 * 4) {
                ...
                if (t2 >>> 0 !== t2 || t2 >= t1.length)
                  throw H.ioore(t1, t2);
                tile = J.$index$asx(t1[t2], J.$tdiv$n(t3, 16));
                t3 = tile.get$creep() - 1;
                tile.creep = t3;
                if (t3 < 0)
                  tile.creep = 0;
  ...
  Renderer: {
    ...
    updateRect$2: function(width, height) {
      ...
      t2 = t1.offsetLeft;
      t3 = t1.offsetTop;
      H.setRuntimeTypeInfo(new P.Rectangle(t2, t3, t1.offsetWidth, t1.offsetHeight), [null]);
      this.top = t3;
      t3 = t1.offsetLeft;
      H.setRuntimeTypeInfo(new P.Rectangle(t3, t1.offsetTop, t1.offsetWidth, t1.offsetHeight), [null]);
      this.left = t3;
      t3 = t1.offsetLeft;
      t2 = t1.offsetTop;
      H.setRuntimeTypeInfo(new P.Rectangle(t3, t2, t1.offsetWidth, t1.offsetHeight), [null]);
      t3 = t1.offsetLeft;
      t4 = t1.offsetTop;
      t5 = t1.offsetWidth;
      t6 = t1.offsetHeight;
      H.setRuntimeTypeInfo(new P.Rectangle(t3, t4, t5, t6), [null]);
      if (typeof t2 !== "number")
        throw t2.$add();
      if (typeof t6 !== "number")
        throw H.iae(t6);
      this.bottom = t2 + t6;
      t6 = t1.offsetLeft;
      H.setRuntimeTypeInfo(new P.Rectangle(t6, t1.offsetTop, t1.offsetWidth, t1.offsetHeight), [null]);
      t2 = t1.offsetLeft;
      t5 = t1.offsetTop;
      t4 = t1.offsetWidth;
      H.setRuntimeTypeInfo(new P.Rectangle(t2, t5, t4, t1.offsetHeight), [null]);
      if (typeof t6 !== "number")
        throw t6.$add();
      if (typeof t4 !== "number")
        throw H.iae(t4);
                ...
                if (displayObject.rotation !== 0) {
                  ...
                  if (t5) {
                    ...
                    if (typeof t16 !== "number")
                      throw H.iae(t16);
                    J.drawImageScaledFromSource$9$x(t6, t7, t12 * t13, t5 * t9, t13, t9, -t13 * t14 * t15 * t10, -t9 * t8 * t11 * t10, t13 * t15 * t16, J.$mul$n(J.$mul$n(displayObject.size.y, displayObject.scale.y), $.game.zoom));
                  } else {
                    t5 = t9.x;
                    if (typeof t5 !== "number")
                      throw t5.$negate();
                    ...
                    if (typeof t14 !== "number")
                      throw H.iae(t14);
                    J.drawImageScaled$5$x(t6, t7, -t5 * t12 * t13 * t10, -t9 * t8 * t11 * t10, t5 * t13 * t14, J.$mul$n(J.$mul$n(displayObject.size.y, displayObject.scale.y), $.game.zoom));
                  }
    ...
    Renderer$3: function(view, width, height) {
      ...
      for (t1 = this.layers, i = 0; i < 20; ++i)
        t1[i] = P.List_List(null, U.DisplayObject);
    },
    static: {Renderer$: function(view, width, height) {
        var t1 = new U.Renderer(view, null, null, null, null, null, P.List_List(20, [J.JSArray, U.DisplayObject]));
        ...
        for (i = $.get$Sporetower_sporetowers().length - 1; i >= 0; --i) {
          ...
          if (t4 <= 0) {
            if (i >= t2)
              throw H.ioore(t1, i);
            $.engine.toString;
            t3.sporeCounter = (Math.random() * 5001 >>> 0) + 7500;
            t1 = $.get$Sporetower_sporetowers();
            if (i >= t1.length)
              throw H.ioore(t1, i);
            t1[i].spawn$0();
  ...
  UISymbol: {
    ...
    static: {"": "UISymbol_activeSymbol,UISymbol_symbols", UISymbol_reset: function() {
        ...
        var symbol, t1;
        symbol = new U.UISymbol(null, keyCode, false, false, template);
        t1 = new P.Rectangle(position.x, position.y, 80, 55);
        H.setRuntimeTypeInfo(t1, [null]);
        symbol.rectangle = t1;
  ...
  World: {
    ...
    static: {"": "World_creeperCounter,World_creeperDirty", World_update: function() {
        var t1, t2, i, j, i0, t3, t4, t5, j0;
        t1 = $.World_creeperCounter;
        t2 = $.game.speed;
        if (typeof t1 !== "number")
          throw t1.$add();
        t2 = t1 + 1 * t2;
        $.World_creeperCounter = t2;
        if (t2 >= 25) {
          $.World_creeperCounter = t2 - 25;
          $.World_creeperDirty = true;
          i = 0;
          while (true) {
            t1 = $.game.world.size.x;
            if (typeof t1 !== "number")
              throw H.iae(t1);
            if (!(i < t1))
              break;
...
function dart_precompiled($collectedClasses) {
  ...
  function Rect(position, size, lineWidth, color, anchor, scale, layer, visible) {
    this.position = position;
    this.size = size;
  ...
  function Game(tileSize, seed, terraformingHeight, speed, zoom, running, mode, paused, won, ghosts, world, scroll, mouseScrolling, keyScrolling, stopwatch, tfLine1, tfLine2, tfLine3, tfLine4, tfNumber, targetCursor) {
  ...
  function Ship(speed, targetPosition, type, status, remove, hovered, selected, maxEnergy, energy, trailCounter, weaponCounter, flightCounter, home, sprite, targetSymbol, selectedCircle, energyRect) {
    this.speed = speed;
    this.targetPosition = targetPosition;
    this.type = type;
    this.status = status;
  ...
  function UISymbol(rectangle, keyCode, active, hovered, building) {
